...
class HMSRB(nn.Module):
    def __init__(self, index, n_feats, baseWidth=32, scale = 4):
        ...
        for i in range (self.scale):
            shrinks.append(nn.Conv2d(64, 32, kernel_size=1, bias=True))
        self.shrinks = nn.ModuleList(shrinks)
        
        convs = []
        for i in range(self.scale):
            convs.append(nn.Conv2d(32, 64, kernel_size=3, stride = 1, padding=1, bias=True))
        self.convs = nn.ModuleList(convs)

        self.conv3 = nn.Conv2d(256, 64, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        sp = self.relu(self.convs[0](self.shrinks[0](x)))
        out = sp
        
        for k in range (self.scale-1):
            sp = sp+x
            sp = self.relu(self.convs[k+1](self.shrinks[k+1](sp)))
            out = torch.cat((out, sp), 1)
#            out = out+sp
        
        return torch.cat((self.SCA(self.conv3(out))+x,x0),1)
...
class DDHRNRK4(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(DDHRNRK4, self).__init__()  
